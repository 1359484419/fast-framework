...
public class RedisClient {
     ...
     * get set
     */
    public <T> Set<T> pullSet(String key) {
   ...
   /* public int deleteKey(String key){
    ...
    public Double zincrementScore(String key, String member, double score,Long second) {
        if(-1==ttl(key)){
            expire(key,second.intValue());
        }
    ...
    //获取zset 当前分值
    public Double zsetCurrentScore(String key, String member) {
        return template.opsForZSet().incrementScore(key, member, 0.0);
    }
